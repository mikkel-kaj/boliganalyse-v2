...
export const GET_SUBJECTS_TOOL_DEFINITION: ToolDefinition = {
  name: "get_subjects",
  description:
    "Get subjects from Danmarks Statistik API. Returns a list of subjects or sub-subjects.",
  input_schema: {
    type: "object",
    properties: {
      subjects: {
        type: "array",
        description:
          "Optional list of subject codes. If provided, fetches sub-subjects for these subjects.",
        items: {
          type: "string",
        },
      },
      includeTables: {
        type: "boolean",
        description:
          "If true, includes tables in the result under each subject.",
      },
      recursive: {
        type: "boolean",
        description:
          "If true, fetches sub-subjects (and tables) recursively through all levels.",
      },
      omitInactiveSubjects: {
        type: "boolean",
        description:
          "If true, omits subjects/sub-subjects that are no longer updated.",
      },
      lang: {
        type: "string",
...
export const GET_TABLES_TOOL_DEFINITION: ToolDefinition = {
  name: "get_tables",
  description:
    "Get tables from Danmarks Statistik API. Returns a list of tables, optionally filtered by subjects.",
  input_schema: {
    type: "object",
    properties: {
      subjects: {
...
export const GET_TABLE_INFO_TOOL_DEFINITION: ToolDefinition = {
  name: "get_table_info",
  description:
    "Get table metadata from Danmarks Statistik API. Returns detailed information about a specific table (variables, value codes, etc.)..",
  input_schema: {
    type: "object",
    properties: {
      table_id: {
...
export const GET_DATA_TOOL_DEFINITION: ToolDefinition = {
  name: "get_data",
  description:
    "Get data from Danmarks Statistik API. Returns data from a specific table, optionally filtered by variables.",
  input_schema: {
    type: "object",
    properties: {
      table_id: {
      ...
      variables: {
        type: "array",
        description:
          "List of filters for data selection. IMPORTANT: Always use the exact variable codes (not display names) from get_table_info results. For example, use 'TAL' if that's the code, even if the display name is 'enhed'.",
        items: {
          type: "object",
          properties: {
            code: {
              type: "string",
              description: "Variable code exactly as shown in get_table_info response (e.g., 'OMRÅDE', 'TAL', 'EJENDOMSKATE'). NOT the display name. Request only one value for TAL at a time",
            },
            values: {
              type: "array",
              description: "List of valid value codes for this variable. Only use codes that exist in get_table_info results.",
              items: {
                type: "string",
              },
            },
      ...
      timeOrder: {
        type: "string",
        description:
          "Optional string for sorting time series ('Ascending' or 'Descending').",
        enum: ["Ascending", "Descending"],
      },
      valuePresentation: {
        type: "string",
        description:
          "Optional string to control value presentation ('Code' or 'Text').",
        enum: ["Code", "Text"],
      },
      lang: {
        type: "string",
        description:
          "Language code for metadata in result ('da' or 'en', default 'da').",
        enum: ["da", "en"],
      },
    },
    required: ["table_id"],
...
export class GetTableInfoTool extends BaseTool {
  ...
  protected async executeImpl(params: Record<string, any>): Promise<string> {
    ...
    try {
      ...
      logger.error(
        `Error fetching table info for ${table_id} from DST API:`,
        error,
      );
      throw error;
    }
...
export class GetDataTool extends BaseTool {
  ...
  protected async executeImpl(params: Record<string, any>): Promise<string> {
    ...
    if (variables) {
      ...
      for (const variable of variables) {
        if (!variable.code || !variable.values) {
          throw new Error(
            "Each variable must have 'code' and 'values' properties",
          );
        }
    ...
    try {
      logger.info(
        `Fetching data from DST API for table ${table_id}: ${
      ...
      if (formatUpper === "JSON" || formatUpper === "JSONSTAT") {
        ...
        ["CSV", "PX", "TSV", "HTML5", "HTML5INCLNOTES"].includes(formatUpper)
      ) {
      ...
      logger.error(
        `Error fetching data for table ${table_id} from DST API:`,
        error,
      );
      throw error;
    }
