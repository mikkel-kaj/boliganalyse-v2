...
The `supabase db push` CLI insists on TLS. The supabase-db container has
`ssl=on` (config in `docker-compose.app.yml`, cert generated by
`setup-postgres-tls.sh` in step 2a). Tunnel to port **5433** on the VPS
— that's the loopback-only mapping for supabase-db. Port 5432 on the
host is taken by `supabase-pooler`, which doesn't speak TLS.

### 2a. (First deploy only) Provision the postgres TLS cert

If `/opt/supabase-stack/postgres-tls/server.crt` already exists on the
VPS, skip to 2b.

```bash
# Push the override that turns ssl=on:
./deploy/scripts/sync-compose.sh boliganalyse

# Generate a self-signed cert (idempotent):
ssh boliganalyse 'bash -s' < ./deploy/scripts/setup-postgres-tls.sh

# Recreate supabase-db so it picks up ssl=on + the cert mount:
...
The cert is self-signed; the SSH tunnel below already protects the wire,
so clients use `sslmode=require` (not `verify-full`).

### 2b. Push migrations

```bash
# Terminal 1 — tunnel to the db (loopback-bound 5433 on the VPS):
ssh -fN -L 5433:localhost:5433 boliganalyse

# Terminal 2 — pull the password and run:
ssh boliganalyse 'sudo grep ^POSTGRES_PASSWORD= /opt/supabase-stack/.env'
...
**Verify:** the script prints `Done.` Re-running is idempotent — applied
migrations are tracked in `supabase_migrations.schema_migrations`.

Close the tunnel when finished:

```bash
pkill -f 'ssh -fN -L 5433:localhost:5433 boliganalyse' || true
```
